...
author:
  - "Qi Gao"
package: splatter
date: "Last updated: March 11, 2023"
...
SifiNet can be installed from Github:
...
Suppose we already have a matrix of count data that we wish to find feature gene sets from. 
Here is a toy example generated with the `complexSCsimulation` package:
...
sifi_obj <- create_SiFINeT_object(counts = data,
                                  gene.name = NULL,
                                  meta.data = NULL,
                                  data.name = NULL,
                                  sparse = FALSE,
...
After building the co-expression network, we further perform additional gene filtering steps to improve the quality of the co-
expression network. Greater values of arguments `t1`, `t2`, and `t3` would lead to a less strict filtering, keeping more genes in the network. The results are saved in `sifi_obj@kset`.
